...
public abstract class Browser {
	...
	public void initProcessInfo() {
		...
		browsePI = new ProcessInfoPOS(m_process.getName(), m_Browse.getAD_Process_ID());
	 ...
	 * @param pi
	 */
	public void setBrowseProcessInfo(ProcessInfoPOS pi) {
		browsePI = pi;
	}
		...
		try {
			...
			for (int id :  getAxisRecordIds(tableName, whereClause)) {
				...
				if (viewColumn.get_ID() > 0)
					browseField.setAD_ViewColumn_ID(viewColumn.getAD_View_Column_ID());
