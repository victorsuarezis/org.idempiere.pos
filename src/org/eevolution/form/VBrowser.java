...
import org.compiere.apps.ProcessCtlPOS;
...
public class VBrowser extends Browser implements ActionListener, ListSelectionListener, ASyncProcess {
	...
	private void initComponents() {
		...
		collapsibleSearch = new CollapsiblePanelPOS(Msg.getMsg(Env.getCtx(),("SearchCriteria")));
		...
		if (getAD_Process_ID() > 0 && getSelectedKeys() != null)
		{
			...
			if(processParameterPanel.validateParameters() == null) {
				...
				if(processParameterPanel.saveParameters() == null) {
					//	Get Process Info
					ProcessInfoPOS pi = processParameterPanel.getProcessInfo();
					//	Set Selected Values
					...
					// Execute Process
					ProcessCtlPOS worker = new ProcessCtlPOS((IProcessUI)this, getWindowNo() , pi , null);
					//	
	...
	public LinkedHashMap<String, GridField> getPanelParameters() {
		...
		for (Entry<String, Object> entry : searchPanel.getParameters().entrySet()) {
			VEditorPOS editor = (VEditorPOS) entry.getValue();
			//	BR [ 251 ]
			if(!((Component)editor).isVisible()
					|| editor.getFieldPOS().isInfoOnly())
				continue;
			//	
			GridField field = editor.getFieldPOS();
