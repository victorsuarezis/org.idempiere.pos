...
import org.compiere.minigrid.CheckRendererPOS;
import org.compiere.minigrid.IDColumn;
import org.compiere.minigrid.IDColumnEditor;
import org.compiere.minigrid.IDColumnRendererPOS;
import org.compiere.minigrid.MiniCellEditor;
import org.compiere.minigrid.MiniCellEditorPOS;
...
public class POSTable extends MiniTable {
	...
	//	logger.config( "MiniTable.setColumnClass - " + index, c.getName() + ", r/o=" + readOnly);
		...
		if (c == IDColumn.class)
		{
			IDColumnRendererPOS columnRenderer = new IDColumnRendererPOS(multiSelection);
			tableColumn.setCellRenderer(columnRenderer);
			if (multiSelection)
			{
				VHeaderRendererPOS headerRenderer = new VHeaderRendererPOS(multiSelection);
			...
			else
			{
				...
				tableColumn.setHeaderRenderer(new VHeaderRendererPOS(DisplayType.Number));
			}
		...
		{
			CheckRendererPOS checkRenderer = new CheckRendererPOS();
			tableColumn.setCellRenderer(checkRenderer);
			if (readOnly)
			{
				tableColumn.setCellEditor(new ROCellEditor());
				tableColumn.setHeaderRenderer(new VHeaderRendererPOS(DisplayType.YesNo));
			}
			else
			{
				if (multiSelection)
		...
		else if (DisplayType.Date == displayType || DisplayType.DateTime == displayType ||  c == Timestamp.class )
		{
			if(DisplayType.DateTime == displayType)
				tableColumn.setCellRenderer(new VCellRendererPOS(DisplayType.DateTime));
			else 
				tableColumn.setCellRenderer(new VCellRendererPOS(DisplayType.Date));
			
			if (readOnly)
				tableColumn.setCellEditor(new ROCellEditor());
			else if (DisplayType.Date == displayType || DisplayType.DateTime == displayType)
				tableColumn.setCellEditor(new MiniCellEditorPOS(c, displayType));
			else 
				tableColumn.setCellEditor(new MiniCellEditor(c));
			
			minWidth.add(new Integer(30));
			if (DisplayType.DateTime == displayType)
				tableColumn.setHeaderRenderer(new VHeaderRendererPOS(DisplayType.DateTime));
			else 
				tableColumn.setHeaderRenderer(new VHeaderRendererPOS(DisplayType.Date));
		}
		//  Amount
		else if (DisplayType.Amount == displayType || c == BigDecimal.class )
		{
			tableColumn.setCellRenderer(new VCellRendererPOS(DisplayType.Amount));
			if (readOnly)
			{
				tableColumn.setCellEditor(new ROCellEditor());
		...
		{
			tableColumn.setCellRenderer(new VCellRendererPOS(DisplayType.Number));
			if (readOnly)
			{
				tableColumn.setCellEditor(new ROCellEditor());
				minWidth.add(new Integer(70));
		...
		{
			tableColumn.setCellRenderer(new VCellRendererPOS(DisplayType.Integer));
			if (readOnly)
				tableColumn.setCellEditor(new ROCellEditor());
			else
				tableColumn.setCellEditor(new MiniCellEditor(c));
		...
		{
			tableColumn.setCellRenderer(new VCellRendererPOS(DisplayType.String));
			if (readOnly)
				tableColumn.setCellEditor(new ROCellEditor());
			else
			...
			tableColumn.setHeaderRenderer(new VHeaderRendererPOS(DisplayType.String));
		}
	//	logger.fine( "Renderer=" + tc.getCellRenderer().toString() + ", Editor=" + tc.getCellEditor().toString());
	}

	public void setRowChecked(int row, boolean b) {
		System.out.println("Method no implemented yet");
	}
}