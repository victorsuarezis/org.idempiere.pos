...
import org.compiere.interfaces.ServerPOS;
...
public final class AEnvPOS
{
	 ...
	 */
	public static void showCenterScreen(FormFramePOS frame) {
	...
	public static boolean actionPerformed (String actionCommand, int WindowNo, Container c)
	{
		...
		{
			PrintScreenPainter.printScreen (EnvPOS.getFrame(c));
		}
		else if (actionCommand.equals("ScreenShot"))
		{
			ScreenShot.createJPEG(EnvPOS.getFrame(c), null);
		}
	...
	//	}
		else if (actionCommand.equals("Exit"))
		{
			...
			{
				AMenu aMenu = (AMenu)EnvPOS.getWindow(0);
				aMenu.dispose() ;
			}
		}
		...
		//  View Menu   ------------------------
		else if (actionCommand.equals("InfoProduct") && AEnvPOS.canAccessInfo("PRODUCT"))
		{
			org.compiere.apps.search.Info.showProduct (EnvPOS.getFrame(c), WindowNo);
		}
		else if (actionCommand.equals("InfoBPartner") && AEnvPOS.canAccessInfo("BPARTNER"))
		{
			org.compiere.apps.search.Info.showBPartner (EnvPOS.getFrame(c), WindowNo);
		}
		else if (actionCommand.equals("InfoAsset") && AEnvPOS.canAccessInfo("ASSET"))
		{
			org.compiere.apps.search.Info.showAsset (EnvPOS.getFrame(c), WindowNo);
		}
		else if (actionCommand.equals("InfoAccount") && 
				  MRole.getDefault().isShowAcct() &&
				  AEnvPOS.canAccessInfo("ACCOUNT"))
		{
			new org.compiere.acct.AcctViewer();
		}
		else if (actionCommand.equals("InfoSchedule") && AEnvPOS.canAccessInfo("SCHEDULE"))
		{
			new org.compiere.apps.search.InfoSchedule (EnvPOS.getFrame(c), null, false);
		}
		//FR [ 1966328 ] 
		else if (actionCommand.equals("InfoMRP") && AEnvPOS.canAccessInfo("MRP"))
		{
			CFrame frame = (CFrame) EnvPOS.getFrame(c);
			int	m_menu_id = MMenu.getMenu_ID("MRP Info");
			AMenu menu = AEnvPOS.getAMenu(frame);
			AMenuStartItem form = new AMenuStartItem (m_menu_id, true, Msg.translate(Env.getCtx(), "MRP Info"), menu);		//	async load
			form.start();
		}
		else if (actionCommand.equals("InfoCRP") && AEnvPOS.canAccessInfo("CRP"))
		{
			CFrame frame = (CFrame) EnvPOS.getFrame(c);
		...
		else if (actionCommand.equals("InfoOrder") && AEnvPOS.canAccessInfo("ORDER"))
		{
			org.compiere.apps.search.Info.showOrder (EnvPOS.getFrame(c), WindowNo, "");
		}
		else if (actionCommand.equals("InfoInvoice") && AEnvPOS.canAccessInfo("INVOICE"))
		{
			org.compiere.apps.search.Info.showInvoice (EnvPOS.getFrame(c), WindowNo, "");
		}
		else if (actionCommand.equals("InfoInOut") && AEnvPOS.canAccessInfo("INOUT"))
		{
			org.compiere.apps.search.Info.showInOut (EnvPOS.getFrame(c), WindowNo, "");
		}
		else if (actionCommand.equals("InfoPayment") && AEnvPOS.canAccessInfo("PAYMENT"))
		{
			org.compiere.apps.search.Info.showPayment (EnvPOS.getFrame(c), WindowNo, "");
		}
		else if (actionCommand.equals("InfoCashLine") && AEnvPOS.canAccessInfo("CASHJOURNAL"))
		{
			org.compiere.apps.search.Info.showCashLine (EnvPOS.getFrame(c), WindowNo, "");
		}
		else if (actionCommand.equals("InfoAssignment") && AEnvPOS.canAccessInfo("RESOURCE"))
		{
			org.compiere.apps.search.Info.showAssignment (EnvPOS.getFrame(c), WindowNo, "");
		}
		...
		{
			showWindow(EnvPOS.getWindow(0));
		}

		//  Tools Menu  ------------------------
		else if (actionCommand.equals("Calculator"))
		{
			Calculator calc = new org.compiere.grid.ed.Calculator(EnvPOS.getFrame(c));
			calc.setDisposeOnEqual(false);
			AEnvPOS.showCenterScreen (calc);
		}
		else if (actionCommand.equals("Calendar"))
		{
			AEnvPOS.showCenterScreen (new org.compiere.grid.ed.Calendar(EnvPOS.getFrame(c)));
		}
		else if (actionCommand.equals("Editor"))
		{
			AEnvPOS.showCenterScreen (new org.compiere.grid.ed.Editor(EnvPOS.getFrame(c)));
		}
		else if (actionCommand.equals("Script"))
		{
			new BeanShellEditor(EnvPOS.getFrame(c));
		}
		else if (actionCommand.equals("Preference"))
		{
			if (role.isShowPreference()) {
				AEnvPOS.showCenterScreen(new Preference (EnvPOS.getFrame(c), WindowNo));
			}
		}
		...
		{
			ADialog.createSupportEMail(EnvPOS.getFrame(c), EnvPOS.getFrame(c).getTitle(), "\n\n");
		}
		else if (actionCommand.equals("About"))
		{
			AEnvPOS.showCenterScreen(new AboutBox(EnvPOS.getFrame(c)));
		}
		else
			return false;
		//
	 ...
	 * @param frame
	 */
	public static void addToWindowManager(FormFramePOS frame) {
		if(frame.isDialog())
			return;
	...
	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(AEnvPOS.class);
	...
	public static void cacheReset (String tableName, int Record_ID)
	{
		...
		if (isServerActive())
		{
			...
			try
			{
				ServerPOS server = (ServerPOS) CConnection.get().getServer();
				if (server != null)
				{
	...
	{
		Set<Window> updated = EnvPOS.updateUI();
		JFrame top = EnvPOS.getWindow(0);
		if (top instanceof AMenu)
		{
